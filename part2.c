...
void execute_instruction(Instruction instruction,Processor *processor,Byte *memory) {
    ...
    int opcode = instruction.opcode; //Obtenemos el opcode de la instruccion.
...
void execute_rtype(Instruction instruction, Processor *processor) {
  ...
  switch (function3){
    ...
    case 0x06:
        switch(function7){
            case 0x00:
              ...
              processor->R[rd] = or1 | or2;
              break;

            case 0x01:
              ;
              unsigned int rem1 = processor->R[rs1];
              unsigned int rem2 = processor->R[rs2];
    ...
    case 0x07:
      ;
      unsigned int and1 = processor->R[rs1];
      unsigned int and2 = processor->R[rs2];
...
void execute_jalr(Instruction instruction, Processor *processor) {
  ...
  processor->PC = processor->R[rs1] + imm;
}
...
int check(Address address, Alignment alignment) {
    if (address > 0 && address < MEMORY_SPACE) {
        ...
        // half align
        else if (alignment == LENGTH_HALF_WORD) return ((address % 2) == 0);
        // word align
        else if (alignment == LENGTH_WORD) return ((address % 4) == 0);
    }
    return 0;
}
